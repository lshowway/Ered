...
from luke_modeling import LukeForRelationClassification
from model_utils import ModelArchive

from util import NullLogger, CometLogger
from data_utils import HEAD_TOKEN, TAIL_TOKEN, MASK_TOKEN
...
def evaluate(args, model, fold="dev", output_file=None):
    ...
    print('evaluated samples: ', len(labels))
...
def main():
    ...
    # word embedding
    args.model_config.vocab_size += 2  # 50265+2
    word_emb = model_weights["luke.embeddings.word_embeddings.weight"]  # 50265*768
    head_emb = word_emb[tokenizer.convert_tokens_to_ids(["@"])[0]].unsqueeze(0)
    tail_emb = word_emb[tokenizer.convert_tokens_to_ids(["#"])[0]].unsqueeze(0)
    model_weights["luke.embeddings.word_embeddings.weight"] = torch.cat([word_emb, head_emb, tail_emb]) # 50267*1024
    tokenizer.add_special_tokens(dict(additional_special_tokens=[HEAD_TOKEN, TAIL_TOKEN]))
    # entity identifier embedding
    ...
    model_weights["luke.entity_identifier_embeddings.entity_identifier_embeddings.weight"] = torch.cat([entity_emb[:1], mask_emb])  # 3*256
    # entity embedding
    model_weights['luke.entity_embeddings.k_entity_embeddings.weight'] = entity_emb

    # load model
    from data_utils import DatasetProcessor
    processor = DatasetProcessor()
    label_list = processor.get_label_list(args.data_dir)  # 9
    model = LukeForRelationClassification(args, num_labels=len(label_list))
