...
class InputFeatures(object):
    def __init__(self, input_ids, entity_ids, input_mask, entity_mask, segment_ids, label_id):
...
class QualityControlProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, dataset_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            query, query_mention, query_offset, query_entry, query_des = query_and_nn.split(' [SEP] ')
            keyword, domain, keyword_mention, key_offset, key_entry, key_des = key_and_nn.split(' [SEP] ')
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(guid=guid,
                             ...
                             mention_a=query_mention, mention_b=None,
...
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        if input_mode == "sentence_pair":
            features = convert_examples_to_features_sentence_pair(args, examples, label_list, args.max_seq_length,
                                                                    tokenizer, output_mode,
                                                                    cls_token=tokenizer.cls_token,
                                                                    sep_token=tokenizer.sep_token,
                                                                    )
        else:
            features = convert_examples_to_features_single_sentence(args, examples, label_list, args.max_seq_length,
                                                                    tokenizer, output_mode,
                                                                    cls_token=tokenizer.cls_token,
                                                                    sep_token=tokenizer.sep_token,
                                                                    )
    ...
    dataset = TensorDataset(all_input_ids, all_entity_ids, all_input_mask, all_entity_mask, all_segment_ids, all_label_ids)
    return dataset


def convert_examples_to_features_sentence_pair(args, examples, label_list, max_seq_length,
                                                 tokenizer, output_mode,
                                                 cls_token='[CLS]',
                                                 sep_token='[SEP]',
    ...
    E_ID = load_entityID(args.KG_file, entity_pad='PAD')
    features = []
    for (ex_index, example) in enumerate(examples):
        ...
        if example.entity_a_bias != -1:
            ...
            entity_tokens_a = [entity_pad] * len(a_seg_1) + [example.entity_a] * len(a_seg_2) + [entity_pad] * len(
                a_seg_3)
        else:
        ...
        if example.entity_b_bias != -1:
            ...
            entity_tokens_b = [entity_pad] * len(b_seg_1) + [example.entity_b] * len(b_seg_2) + [entity_pad] * len(
                b_seg_3)
        else:
        ...
        entity_ids = [E_ID[x] for x in entity_tokens]
        entity_mask = []
        for x in entity_ids:
            if x == 0:
...
def convert_examples_to_features_single_sentence(args, examples, label_list, max_seq_length,
                                                 tokenizer, output_mode,
                                                 cls_token='[CLS]',
                                                 sep_token='[SEP]',
                                                 sequence_a_segment_id=0,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        entity_tokens_a = entity_tokens_a[: max_seq_length - 2]
        tokens = [cls_token] + tokens_a + [sep_token]
        entity_tokens = [entity_pad] + entity_tokens_a + [entity_pad]
        segment_ids = [sequence_a_segment_id] * len([cls_token] + tokens_a) + [sequence_b_segment_id] * len([sep_token])
        input_ids = tokenizer.convert_tokens_to_ids(tokens)
