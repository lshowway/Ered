...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        if hub_path is not None:
            from KFormers_roberta.utils import file_util
...
class BertModel(BertPreTrainedModel):
    ...
    def convert_idx_to_mask(self, input_ids, attention_mask=None):
        ...
        # It is an implementation of paper: Does Knowledge Help General NLU? An Empirical Study (figure 2)
        # where attention mask is modified
        # since attention mask is too large to cache during data pre-processing phase
        # we save t0, t1, t2, a_idx, b_idx during pre-processing
        # and then recover the attention mask from this five number
        # t0: Upper left matrix in Fig. 2, that is the length of original text
        # t1: Intermediate matrix in Fig. 2, that is the length of query(sentence A) description
        # t2: Lower right matrix in Fig. 2, that is the length of keyword(sentence B) description
        # a_idx: it is the index of entity in query, note that CLS is considered
        # b_idx: it is the index of entity in keyword, not that CLS, query, SEP are considered
...
class BertForSequenceClassificationMultiTask(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.task_dropout_prob)
        self.classifier_multitask = nn.Linear(config.hidden_size, num_labels)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, task_ids=None, labels=None):
        _, pooled_output = self.bert(
            input_ids, token_type_ids, attention_mask, task_ids=task_ids, output_all_encoded_layers=False)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier_multitask(pooled_output)
