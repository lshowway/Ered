...
from utils_2.glue_utils import processors, GLUE_TASKS_NUM_LABELS, \
...
def main():
    ...
    if output_mode == "classification":
        logger.info("  Label list = [{}]".format(", ".join(label_list)))
    ...
    if args.do_train:
        ...
        dev_eval_features = convert_examples_to_features(
            dev_eval_examples, label_list, args.max_seq_length, tokenizer,
    ...
    if args.do_eval:
        ...
        test_eval_features = convert_examples_to_features(
            test_eval_examples, label_list, args.max_seq_length, tokenizer,
            cls_token='<s>' if is_roberta else '[CLS]',
            sep_token='</s>' if is_roberta else '[SEP]',
            output_mode=output_mode,
        )
