...
# from .util import ENTITY_TOKEN, convert_examples_to_features, DatasetProcessor
...
#  load tokenizer
tokenizer = RobertaTokenizer.from_pretrained(args.bert_model_name)
...
# experiment_logger.log_parameters({p.name: getattr(args, p.name) for p in args})
# experiment_logger.log_parameters({p.name: getattr(args, p.name) for p in args})
...
if True: #args.do_train:
    ...
    trainer = Trainer(args, model=model, dataloader=train_dataloader, num_train_steps=num_train_steps, step_callback=step_callback)  # 这儿相当于__init__
