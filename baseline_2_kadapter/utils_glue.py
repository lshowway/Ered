...
relations = ['P1001','P1303','P2094','P412','P410','P449','P495','P123','P1877','P175','P407','P140','P740','P106',
             'P361','P937','P355','P206','P463','P641','P921','P1408','P39','P17','P118','P306','P710','P264','P276',
             'P364','P466','P551','P40','P3450','P150','P84','P241','P1411','P102','P159','P1346','P26','P57','P460',
             'P59','P3373','P674','P527','P105','P6','P27','P800','P156','P127','P136','P86','P22','P991','P750',
             'P177','P131','P178','P931','P1344','P1923','P1435','P400','P101','P31','P135','P137','P176','P974',
             'P706','P25','P58','P413','P403','P155','P4552']
...
class TACREDProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, dataset_type, negative_sample):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = line['label']
            if label == 'NA' and dataset_type == 'train':
...
def convert_examples_to_features_tacred(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = tokenizer.tokenize(text_a[:subj_start])
        subj_special_start = len(tokens)

        tokens += ['@']
        tokens += tokenizer.tokenize(text_a[subj_start:subj_end + 1])
        tokens += ['@']
        tokens += tokenizer.tokenize(text_a[subj_end + 1:obj_start])
        obj_special_start = len(tokens)

        tokens += ['#']
        tokens += tokenizer.tokenize(text_a[obj_start:obj_end + 1])
        tokens += ['#']
        tokens += tokenizer.tokenize(text_a[obj_end + 1:])
